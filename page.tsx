...
export default function Home() {
  ...
  const tellFortune = async () => {
    ...
    try {
      ...
      setResult("占いに失敗しました。設定を確認してください。");
      console.error(error);
    }
    setLoading(false);
  };
  ...
  return (
    <main className="min-h-screen bg-black text-white p-8 font-sans flex flex-col items-center">
      ...
      <div className="bg-gray-900 p-6 rounded-lg shadow-xl w-full max-w-md border border-purple-500">
        <div className="mb-4">
          <label className="block text-sm mb-1">お名前を入力</label>
          <input 
        ...
        <div className="flex gap-4 mb-6">
          <div className="flex-1">
            <label className="block text-sm mb-1">血液型</label>
            <select 
          ...
          <div className="flex-1">
            <label className="block text-sm mb-1">星座</label>
            <select 
              className="w-full p-2 rounded bg-gray-800 border border-gray-700"
        ...
        <button 
          onClick={tellFortune}
          disabled={loading}
          className="w-full bg-purple-600 hover:bg-purple-700 p-3 rounded-lg font-bold transition-all disabled:bg-gray-600"
        >
          {loading ? "鑑定中..." : "鑑定を開始する"}
        </button>
