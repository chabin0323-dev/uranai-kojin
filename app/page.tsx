  ...
  const handleFortune = async () => {
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${process.env.NEXT_PUBLIC_GOOGLE_AI_API_KEY}`, {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: `占い師として、${name}さん（${bloodType}・${zodiac}）を占って。冒頭で「${zodiac}で${bloodType}の${name}さん、こんにちは！」と必ず言ってから、150文字程度で具体的で元気が出るアドバイスをして。` }] }]
        })
      });
      const data = await response.json();
      const aiText = data.candidates?.[0]?.content?.parts?.[0]?.text;
      setResult(aiText || "占い失敗");
      ...
      <div className="w-full max-w-md bg-white/5 p-8 rounded-3xl border border-white/10 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-black">
            <select value={bloodType} onChange={(e) => setBloodType(e.target.value)} className="w-full p-4 rounded-xl outline-none">
              <option value="A型">A型</option><option value="B型">B型</option><option value="O型">O型</option><option value="AB型">AB型</option>
            </select>
            <select value={zodiac} onChange={(e) => setZodiac(e.target.value)} className="w-full p-4 rounded-xl outline-none">
